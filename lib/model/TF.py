...
class GoalTF(nn.Module):
    def __init__(self, d_model=128, dropout=0, N=2, d_ff=256, h=2) -> None:
        super(GoalTF, self).__init__()
        
        attn = MultiHeadAttention(h, d_model, dropout=dropout)
        ...
        self.src_embed =  nn.Sequential(LinearEmbedding(d_model, d_model), deepcopy(position))

    def forward(self, src, tgt, src_mask=None, tgt_mask=None, query_pos=None):
        """
