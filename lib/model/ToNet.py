...
class GoalNet(nn.Module):
    def __init__(self, config):
        ...
        # self.goal_tf = GoalTF(d_model=n_actor, N=6)
        self.tf = GoalTF(d_model=n_actor, N=6) #nn.Transformer(d_model=n_actor)
    ...
    def forward(self, actors: Tensor, actor_idcs, actor_ctrs) -> Tensor:
        ...
        out = self.tf(goal_input, query_batches)
